...
// RF-GH-026: branch of a commit or a feature, as a neutral F0TagRaw. With
// `href`, it links to Activity filtered by that branch.
export function BranchTag({ name, href, title, className }: { name: string; href?: string; title?: string; className?: string }) {
  ...
  if (href) {
    return (
      <Link href={href} className={classes} title={title ?? `View activity on ${name}`}>
        {content}
      </Link>
    );
  }
