module Auth
  class GithubLogin
    ...
    # RF-TEAM-014: adds the GitHub identity to an existing account (e.g. one
    # created with email or Google). It does not look up by email: the
    # account is the session's. It fails if the identity belongs to another
    # account.
    def self.link(code:, user:)
      profile = fetch_profile(exchange_code(code))

      owner = User.where(github_uid: profile["id"]).first
      raise IdentityTaken, "this GitHub account is already linked to another user" if owner && owner.id != user.id
