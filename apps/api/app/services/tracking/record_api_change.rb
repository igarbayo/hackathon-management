# RF-API-006: si una escritura hecha con un token (API o MCP) no genera ya
# su propio evento (feature_status_changed, feature_assigned…), se deja
# constancia con system/api_change y la lista de campos, no sus valores.
# Se llama solo cuando hay token (las escrituras de la web no llevan via).
module Tracking
  class RecordApiChange
    def self.call(team:, membership:, resolved_token:, entity:, key:, fields:, channel:)
      return if fields.blank?

      ActivityEvent.create!(
        team_id: team.id,
        source: "system",
        kind: "api_change",
        dedupe_key: "api:#{SecureRandom.uuid}",
        occurred_at: Time.current,
        actor: actor_hash(membership, resolved_token),
        title: "#{entity} #{key} editado por #{channel == 'mcp' ? 'MCP' : 'API'}",
        payload: { "entity" => entity, "key" => key.to_s, "fields" => fields },
        via: resolved_token.via(channel: channel)
      )
    end

    # Con un token de integración el actor es la propia integración, no una
    # persona (12-acceso-programatico.md#tokens-de-integración-de-equipo).
    def self.actor_hash(membership, resolved_token)
      if membership
        { "user_id" => membership.user_id.to_s, "membership_id" => membership.id.to_s, "display" => membership.display_name }
      else
        token = resolved_token.token_record
        { "user_id" => nil, "membership_id" => nil, "integration_id" => token.id.to_s, "display" => "#{token.name} (integración)" }
      end
    end
  end
end
