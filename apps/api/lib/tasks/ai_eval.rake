...
# ejecuta contra el prompt/proveedor reales (hace falta GEMINI_API_KEY) antes
# de subir prompt_version. No corre en CI: llama a la IA de verdad y no es
# un test de comportamiento del código, es una evaluación de la calidad del
# prompt. La lógica vive en AiEval::Runner para poder probarla sin el exit.
namespace :ai do
  ...
  task eval: :environment do
    ...
    result = AiEval::Runner.call(provider: Ai::ProviderFactory.build, system_prompt: system_prompt, schema: schema)
