...
| Is every file in the repository free software? | All code, documentation and configuration is. The **logos and icons are not**: they are all rights reserved (section 6). |
...
and their notices must be kept. Each file copied or adapted from another project carries an
SPDX header naming the upstream author and license, and the full license text is in
[`LICENSES/`](../LICENSES/). The rest of the repository is declared in
[`REUSE.toml`](../REUSE.toml) (see section 8.4).

<!-- REUSE-IgnoreStart -->
| Component | Where | Origin and license | Status |
|---|---|---|---|
| **`DatePicker`** | `apps/web/src/components/ui/date-picker.tsx` | Adapted from [a-good-date-picker](https://github.com/gulipad/a-good-date-picker), MIT, Copyright (c) 2025 Guli Moreno | SPDX header with both copyright holders and `MIT AND AGPL-3.0-or-later`; MIT text in `LICENSES/MIT.txt`. |
| **shadcn/ui primitives** | `apps/web/src/components/ui/*.tsx` | Generated by the `shadcn` CLI from [shadcn/ui](https://github.com/shadcn-ui/ui), MIT, Copyright (c) 2023 shadcn | Same SPDX header in each of the 22 files (every file in the folder except `date-picker.tsx`). |
| **F0 design tokens** | `apps/web/src/app/globals.css` | Copied from `@factorialco/f0-core@2.7.0`, which says `"license": "MIT"`, author Factorial | SPDX header attributing the tokens to Factorial under MIT. The upstream repository and npm package contain no `LICENSE` file, only the `license` field. Colour and size values are unlikely to be protected by copyright, but the MIT attribution is kept to be safe. |
...
| **HackBoard logos and icons** | `apps/web/public/*.svg`, `apps/web/public/icons/*`, `apps/web/src/app/apple-icon.png`, `favicon.ico`, `icon.svg` | Original work, Copyright (c) 2026 Ignacio Garbayo | **All rights reserved** (`LicenseRef-AllRightsReserved`, text in `LICENSES/`). They are **not** under AGPL. They may be used unmodified to refer to the official project; a fork or a modified deployment must replace them with its own artwork. `opengraph-image.tsx` is AGPL code that reads the logo file at build time, so a fork that swaps the SVG gets its own image. |
| **Google and GitHub logos** | `apps/web/src/components/auth/provider-logos.tsx` | Google "G" logo, trademark of Google LLC; GitHub mark from [Octicons](https://github.com/primer/octicons) (`mark-github`), trademark of GitHub, Inc. (Octicons' MIT license does not cover GitHub's logos) | **Not licensed** (`LicenseRef-ThirdPartyLogos`, text in `LICENSES/`). Used unmodified, only on the "Continuar con Google/GitHub" buttons, as the [Sign in with Google Branding Guidelines](https://developers.google.com/identity/branding-guidelines) and [GitHub Logos](https://github.com/logos) guidelines allow. SPDX header in the file. |
<!-- REUSE-IgnoreEnd -->
...
2. **Optional: replace `redis:7` with `valkey/valkey:8`** in `docker-compose.yml` for a
   development stack that is fully OSI-approved.
3. **Before the first external contribution, decide on DCO or CLA.** A DCO keeps the
...
### 8.4 REUSE compliance

The repository follows the [REUSE 3.3 specification](https://reuse.software/spec-3.3/).
Every file has a machine-readable copyright holder and license:

| Files | How they are declared | License |
|---|---|---|
| Everything by default | [`REUSE.toml`](../REUSE.toml), `path = "**"` | `AGPL-3.0-or-later` |
| Code copied or adapted from other projects (section 6) | SPDX header at the top of the file, which takes precedence over `REUSE.toml` | `MIT AND AGPL-3.0-or-later` |
| Logos and icons | `REUSE.toml`, with `precedence = "override"` | `LicenseRef-AllRightsReserved` |

CI runs `reuse lint` on every push, so a file without licensing information fails the build.

**Example: adding code from another project.** Suppose you copy a hook from a repository
under the MIT license. Put this at the top of the file (with the real names), and check that
`LICENSES/MIT.txt` exists (`reuse download MIT` if not):

<!-- REUSE-IgnoreStart -->
```ts
// SPDX-FileCopyrightText: 2024 Original Author
// SPDX-FileCopyrightText: 2026 Ignacio Garbayo
// SPDX-License-Identifier: MIT AND AGPL-3.0-or-later
```
<!-- REUSE-IgnoreEnd -->

Then add a row for it to section 6 of this document.
