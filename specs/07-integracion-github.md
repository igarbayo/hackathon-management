...
- `RF-GH-023` Implementado: al vincular un repo se importa el histórico reciente: los commits de las ramas activas desde `hackathon.starts_at` (máx. 1000 commits en total, los más recientes) y los PRs abiertos. Se hace en un job. "Ramas activas" son las que tienen commits desde `starts_at`: el job lista las ramas del repo (máx. 50 por importación, la por defecto siempre incluida) y pide los commits de cada una. Un commit que está en varias ramas es un solo evento con todas ellas (RF-GH-026); `branch` es una rama de trabajo si la hay, no la por defecto, para que la atribución por rama funcione. Los eventos se crean al momento, pero las stats y ficheros de cada commit (una petición a GitHub por commit) se piden repartidas a 30 por minuto, así que en una importación grande van apareciendo durante unos minutos (RNF-GH-002). Los commits anteriores a `starts_at` nunca se importan ([ADR-0019](decisiones.md#adr-0019)); para traerlos, se adelanta el inicio en ajustes (RF-TEAM-015), que relanza la importación. Si el hackathon no tiene `starts_at` (equipos creados antes de que se pusiera por defecto), no se importa ningún commit.
- `RF-GH-025` Implementado: el resultado de la última importación se guarda en `Repository.last_import` (cuántos commits y PRs, desde qué fecha, o por qué ninguno) y se muestra bajo cada repo en ajustes y en el paso de repo del onboarding: "Importando el histórico…", "87 commits desde el 25 abr 2026 y 0 PRs abiertos." o "El hackathon no tiene fecha de inicio, así que no se han importado commits.". Mientras se importa, la web consulta cada 3 s. El resumen dice también de cuántas ramas son los commits ("12 commits de 3 ramas desde…"). Una importación que no termina (el job se perdió con su Redis o murió con el worker) no se queda en "Importando…" para siempre: `enqueue` y el paso a `running` guardan `expires_at` (15 min después; si GitHub corta por cuota, 15 min después de que vuelva) y, pasado ese momento, la API la devuelve como fallida con `reason: "stalled"`, y la web dice que se atascó y que se resincronice.
- `RF-GH-026` Implementado: se sabe en qué ramas está cada commit. `ActivityEvent.branches` las guarda todas: la importación las rellena y un push de un commit ya conocido (p. ej. al mergear a la rama por defecto) añade la rama sin crear otro evento. En Actividad cada evento de GitHub muestra su rama como etiqueta (y "+N" con las demás en el título), que filtra el feed por esa rama. El detalle de una feature lista sus ramas con commits.
...
| RNF-GH-002 | La API de GitHub se usa con un rate limiting propio por instalación, dejando un margen del 20 % sobre lo que permite GitHub. `Github::RateBudget` guarda en Redis, por instalación, lo último que dijo GitHub (`x-ratelimit-*`): en cuanto queda menos del 20 %, ningún job de esa instalación vuelve a llamar hasta el reset (se reprograman para entonces), sin gastar una petición en descubrirlo. La importación del histórico reparte las peticiones de stats a 30 por minuto (1800/h, un 36 % del cupo de 5000/h), para dejar sitio a los webhooks y a otras importaciones. |
