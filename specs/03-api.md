...
- **Autenticación del CLI y el MCP:** `Authorization: Bearer hb_mt_<token>` (token de miembro).
...
| 400 | `bad_request` |
| 401 | `unauthenticated` |
| 403 | `forbidden` (el rol no basta, dentro de un equipo del que eres miembro) |
| 404 | `not_found` |
| 409 | `conflict` |
...
- [ABIERTO] Recuperación de contraseña por email. Necesita un proveedor de email; en el MVP el login con GitHub es la alternativa.
...
| Método | Ruta | Descripción | Req |
|--------|------|-------------|-----|
| GET | `/teams/:id/activity` | Filtros: `user_id`, `feature_id`, `source`, `kind`, `attribution_status` (`confirmed`\|`suggested`\|`none`), `since`, `until`. Paginado | RF-ACT-001 [F3] |
...
| POST | `/mcp` | token de miembro | Endpoint MCP (Streamable HTTP) | RF-MCP-001 [F5] |
