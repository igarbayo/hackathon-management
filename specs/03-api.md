...
| POST | `/teams/:id/activity/unclaim` | `{event_ids[]}` (máx. 100). Deja los eventos sin usuario (`display` vuelve a `author_name`) y quita sus identidades de `git_identities` del miembro que los tenía; ese miembro queda en `unclaimed_by`. Un miembro solo sobre los suyos; un owner, sobre cualquiera. Nadie sobre sus propios eventos con el login de su cuenta de GitHub (cuentan como omitidos) | RF-ACT-018 |
