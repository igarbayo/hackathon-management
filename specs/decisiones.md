...
- **Decisión:** todo lo que ve una persona en `apps/web` pasa a un inglés sencillo, con el mismo registro que el README: textos, `aria-label`, toasts, placeholders, fechas (`en-US`), `lang="en"`, metadata, Open Graph (`en_US`), JSON-LD (`inLanguage: "en"`), manifest, imagen OG, `/llms.txt` y la política de privacidad. Los comentarios y los tests de `apps/web` también. El `DatePicker` pierde su locale española (RNF-UI-013, [13](13-sistema-diseno.md)).
- **Alternativas:**
  - **i18n con español e inglés (`next-intl` o similar):** sirve a los dos públicos, pero duplica cada texto, obliga a elegir idioma por persona o por equipo y añade una dependencia y un paso más a cada cambio de interfaz. Se puede añadir más adelante sobre los textos en inglés.
  - **Seguir en español:** coherente con las specs, pero deja fuera a la mayoría de los equipos y choca con el README.
- **Consecuencias:** el alcance se amplía después a todo el repo salvo `specs/` y `CLAUDE.md`: la API (mensajes de error, validaciones, títulos de eventos que genera el servidor, "Deleted user", errores y descripciones del MCP), el CLI, los comentarios y los tests de todas las apps, el CI y la configuración. La IA responde en inglés por defecto (`coverage_v2`, `attribution_v2`; se borran las plantillas `v1`, que siguen en el historial de git) y el idioma por equipo sigue `[ABIERTO]` en la spec 06. Los presets de PAT pasan de `observar`/`agente`/`completo` a `observe`/`agent`/`full`: es un cambio de contrato de la API, sin alias, porque el único cliente conocido es la web. Las specs siguen en español ([README](README.md)) y citan textos de la interfaz en español como descripción; el texto real es el inglés del código. Lo ya guardado en la base de datos (títulos de eventos antiguos, actores "Usuario eliminado", análisis ya generados) no se traduce.

## ADR-0021

**Redis corre en la Raspberry Pi (Valkey) en vez de en Upstash** · Aceptado · 2026-09-25

- **Contexto:** en producción, api y worker corren en la Raspberry Pi y Redis estaba en Upstash. Todo lo que Hackboard guarda en Redis es efímero (colas de Sidekiq, contadores de `RateLimiter`, el lock de análisis, la caché de installation tokens de GitHub y las respuestas de `Idempotency-Key`), y cada operación de Sidekiq y de rate limiting cruzaba Internet con TLS hasta Upstash. A la Pi le quedan unos 400 MB de RAM libres.
- **Decisión:** `docker-compose.prod.yml` añade un servicio `redis` con `valkey/valkey:8-alpine` en la misma máquina, sin puerto publicado (solo lo ven api y worker por la red de Compose), con `maxmemory 64mb`, `maxmemory-policy noeviction` (Sidekiq no debe perder trabajos si se llena) y AOF en el volumen `redis_data` para no perder las colas al reiniciar. El contenedor tiene `mem_limit: 96m`. `REDIS_URL` pasa a `redis://redis:6379/0`.
- **Alternativas:**
  - **Seguir en Upstash:** nada que operar, pero añade latencia a cada job y cada petición limitada, depende de la cuota del plan y es un encargado del tratamiento más en la política de privacidad.
  - **`redis:7` en vez de Valkey:** igual de compatible con Sidekiq, pero desde la 7.4 tiene licencia RSALv2/SSPLv1, no OSI; Valkey es BSD-3-Clause y es lo que ya recomendaba `docs/COMPONENTS_LICENSE.md` ([ADR-0016](#adr-0016)).
- **Consecuencias:** Upstash deja de ser encargado del tratamiento (09 y `/privacy`). Hay un servicio más que vigilar en la Pi y sus datos dependen del disco de la Pi, lo que se asume porque nada de lo que hay en Redis es la fuente de verdad. Al migrar solo se pierden los jobs pendientes, programados o en reintento que queden en Upstash, así que hay que cambiar `REDIS_URL` con las colas vacías; `sidekiq-cron` vuelve a cargar su programación al arrancar.
